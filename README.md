...
Claude Code files (`.claude/`) are the canonical source. Copilot and Codex files are generated by `scripts/generate.mjs`.
...
```bash
node scripts/generate.mjs                    # Generate all platforms
node scripts/generate.mjs --platform copilot # Copilot only
node scripts/generate.mjs --platform codex   # Codex only
node scripts/generate.mjs --clean            # Remove generated files
```
