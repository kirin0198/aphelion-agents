...
Phase 7: レビュー         → reviewer       → ⏸ ユーザー承認 → 完了
```
...
`analyst` の `AGENT_RESULT` ブロックを受け取った場合はPhase 3から開始する。
...
- **テスト失敗** → `test-designer` に `tester` の失敗レポートを渡して原因分析 → `developer` に修正フィードバックを渡して修正 → `tester` 再実行 → 承認を求める
...
差し戻し時は以下を `developer` に渡す（`test-designer` が作成した修正フィードバックを含める）:

```
## 修正依頼

### 差し戻し元
{test-designer（テスト失敗分析）/ reviewer}
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  Phase 4 実装             ✅ 承認済み
  Phase 5 テスト設計       ✅ 承認済み ({N} テストケース)
  Phase 6 テスト実行       ✅ 承認済み ({N} テスト通過)
  Phase 7 レビュー         ✅ 承認済み (CRITICAL なし)
...
- [ ] 全フェーズが正常完了した（UIなしの場合は Phase 2 スキップ）
- [ ] 各フェーズでユーザーの承認を得た
