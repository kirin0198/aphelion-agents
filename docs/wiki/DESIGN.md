...
本ドキュメントは Aphelion リポジトリの `wiki/` ディレクトリ（バイリンガル wiki）に関する**軽量な情報アーキテクチャ設計メモ**である。
...
| # | slug | 言語別パス | 主読者 |
|---|------|-----------|--------|
| 1 | `Home` | `wiki/en/Home.md` / `wiki/ja/Home.md` | 全員 |
| 2 | `Getting-Started` | `wiki/en/Getting-Started.md` / `wiki/ja/Getting-Started.md` | 新規ユーザー |
| 3 | `Architecture` | `wiki/en/Architecture.md` / `wiki/ja/Architecture.md` | エージェント開発者 |
| 4 | `Triage-System` | `wiki/en/Triage-System.md` / `wiki/ja/Triage-System.md` | 新規ユーザー / エージェント開発者 |
| 5 | `Agents-Reference` | `wiki/en/Agents-Reference.md` / `wiki/ja/Agents-Reference.md` | エージェント開発者 |
| 6 | `Rules-Reference` | `wiki/en/Rules-Reference.md` / `wiki/ja/Rules-Reference.md` | エージェント開発者 |
| 7 | `Platform-Guide` | `wiki/en/Platform-Guide.md` / `wiki/ja/Platform-Guide.md` | プラットフォーム移植者 |
| 8 | `Contributing` | `wiki/en/Contributing.md` / `wiki/ja/Contributing.md` | エージェント開発者 |
...
**将来の分割判断**: エージェント数が 40 を超える、または個別ページ当たり 300 行超となった場合は `wiki/en/agents/<name>.md` 形式への分割を再検討する。その場合でも Agents-Reference.md は一覧 + 各ページへのリンク集として残す。
...
- `wiki/en/` と `wiki/ja/` 相互のクロスリンクは「言語切替ヘッダー」以外では使わない
...
> EN canonical: <YYYY-MM-DD> of wiki/en/<slug>.md (or commit SHA)
> Audience: <primary reader>
```

- **EN canonical** 行は `wiki/ja/` 側のみで意味を持つ（英語版がどの時点に追随しているかを示す）。英語版では `EN canonical: self` または行を省略してよい
...
- `scripts/generate.py` が `wiki/` を誤って走査しないよう、**既存スクリプトの除外設定を確認のみ行う**（このタスクは developer が Platform-Guide 執筆時に確認し、問題があれば別 issue 化）
...
- 画像 / 図表が必要になった場合は `wiki/assets/` を後続 PR で追加する（今回のスコープ外）
...
Phase 1: 骨格と主要ナビゲーション
  ├─ Task 1-1: wiki/en/Home.md（目次ハブ。全ページへのリンク雛形を先に置く）
  └─ Task 1-2: wiki/ja/Home.md（1-1 と同 PR）

Phase 2: コア解説ページ（canonical からの要約主体）
  ├─ Task 2-1: wiki/en/Architecture.md + wiki/ja/Architecture.md
  ├─ Task 2-2: wiki/en/Triage-System.md + wiki/ja/Triage-System.md
  └─ Task 2-3: wiki/en/Getting-Started.md + wiki/ja/Getting-Started.md

Phase 3: 大型リファレンス（フォーマット統一が重要）
  ├─ Task 3-1: wiki/en/Agents-Reference.md + wiki/ja/Agents-Reference.md
  └─ Task 3-2: wiki/en/Rules-Reference.md + wiki/ja/Rules-Reference.md

Phase 4: 周辺ページ
  ├─ Task 4-1: wiki/en/Platform-Guide.md + wiki/ja/Platform-Guide.md
  └─ Task 4-2: wiki/en/Contributing.md + wiki/ja/Contributing.md
...
- **理由**:
  - 配信先は「リポジトリ内 `wiki/` ディレクトリ」（ISSUE.md §4）であり、GitHub Wiki ではない
