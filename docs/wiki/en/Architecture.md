...
[Maintenance](./Agents-Reference.md#maintenance-domain) ·
[Standalone](./Agents-Reference.md#standalone-agents)

Maintenance is a **fourth flow independent from the primary Discovery → Delivery → Operations pipeline**, invoked via `/maintenance-flow` for existing-project maintenance tasks. Patch and Minor plans complete standalone; only Major plans hand off to Delivery via `MAINTENANCE_RESULT.md`. See [Agents Reference → Maintenance](./Agents-Reference.md#maintenance-domain) and [Triage System → Maintenance Flow Triage](./Triage-System.md#maintenance-flow-triage) for full specs.
