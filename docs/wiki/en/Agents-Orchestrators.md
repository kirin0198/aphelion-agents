...
> **Update history**:
>   - 2026-07-29: doc-reviewer entry — pass/fail vocabulary, STATUS pairing, visual-designer trigger, corrected field list (#173 #174 #184)
...
- **Responsibility**: Reviews cross-document consistency for markdown artifacts (SPEC.md / ARCHITECTURE.md / UI_SPEC.md / VISUAL_SPEC.md / docs/design-notes/, DISCOVERY_RESULT.md). Auto-inserted by flow orchestrators after spec-designer / ux-designer / visual-designer / architect / scope-planner / analyst-core complete (post-insert). Runs in an isolated context independent from the producing agent. Read-only — does not generate or modify any files; emits a text report only.
...
- **AGENT_RESULT fields**: `STATUS`, `DOC_REVIEW_RESULT` (`pass` | `fail` — `fail` is always paired with `STATUS: failure`, since the rollback trigger is the AND of both), `INCONSISTENCY_COUNT`, `ADVISORY_COUNT`, `INFO_COUNT`, `TRIGGERED_BY`, `TARGET_ARTIFACTS`, `INCONSISTENCY_ITEMS`, `NEXT`
- **NEXT conditions**:
