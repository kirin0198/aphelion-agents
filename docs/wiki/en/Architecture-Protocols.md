...
> **Last updated**: 2026-07-29 (updated 2026-07-29: persist HAS_UI / UI_TYPE in DISCOVERY_RESULT.md, #194; add the MAINTENANCE_RESULT.md schema section, #168; 2026-04-25: split from Architecture.md, #42)
...
- `HAS_UI` (true / false) and `UI_TYPE` (web / desktop / cli-tui / none) — persisted from the Discovery triage answer so `delivery-flow` does not have to re-infer it (#194)
...
Generated by `maintenance-flow` on the **Major** plan only. Input for `delivery-flow`,
which validates it at startup and — when `DISCOVERY_RESULT.md` is also present — treats
it as authoritative. Since the analyst chain already ran inside `maintenance-flow`,
`spec-designer` is skipped and the flow starts at `architect` in differential mode.
...
A missing field is a hard error: `delivery-flow` reports `STATUS: error` rather than
falling back to the user interview, which would discard the Major pre-processing.
