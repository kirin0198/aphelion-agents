...
このページはもとの Architecture.md を 3 ページに分割したもの（#42）です。Aphelion の概念的ドメインモデル（3ドメインモデル・セッション分離戦略・PRODUCT_TYPE 分岐）を扱います。プロトコルと運用ルールは兄弟ページを参照してください: [プロトコル](./Architecture-Protocols.md)、[運用ルール](./Architecture-Operational-Rules.md)。
