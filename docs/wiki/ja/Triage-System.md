...
| **Standard** | 複数ファイルプロジェクト | + `scaffolder` + `visual-designer`‡ + `doc-writer` |
...
Phase 2a: UIデザイン       → ux-designer        → doc-reviewer (auto) → 承認  [HAS_UI: true のみ]
Phase 2b: ビジュアルデザイン → visual-designer    → doc-reviewer (auto) → 承認  [HAS_UI: true かつ Standard+ のみ]
...
| `visual-designer` | Standard / Fullかつ `HAS_UI: true` | Delivery |
