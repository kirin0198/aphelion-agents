...
<!--
本メモは #133 の planning doc であり、handoff ブロックは意図的に置いていない。
#211 の refresh 作業でブロックを注入すると `issue_number: 211` が
archive-closed-plans.yml の大文字小文字を区別しない抽出にマッチし、
#133 未実装のまま #211 のクローズで本メモが archived/ へ退避される。
#133 に着手する際は analyst.md の Legacy Resume 経路が #133 でブロックを注入する。
-->
