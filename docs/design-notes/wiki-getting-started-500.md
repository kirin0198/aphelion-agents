...
## §5 深掘り調査結果（analyst-core, 2026-05-31）— ※方針は承認待ちの「案」

> Updated: 2026-05-31 (analyst-core deep analysis; approach is a PROPOSAL pending user approval)
...
### 5.3 対応方針（案 — 承認待ち。複数案を比較）

ローカルでは再現しないため、確定的なコード修正を盲目的に行うのは不適切。以下の段階的アプローチを提案する。

**方針A（推奨）: インフラ確認 → 再デプロイで解消を確認 → 必要時のみコード堅牢化**
1. ユーザーに CF Pages ダッシュボードで以下を確認依頼（analyst-core/architect は権限外）:
   - 最新デプロイの **ビルドログ**（prebuild/sync-wiki.mjs と astro build が成功しているか、
     getting-started の2ファイル生成ログがあるか）
   - **デプロイ済み commit SHA** が現 main HEAD（10e405c 系）と一致するか
   - デプロイ状態が "Success" でアクティブか、過去の失敗デプロイがアクティブのままでないか
2. **空コミット or 再デプロイをトリガ**して最新ソースをクリーンに再ビルド・再配信し、500 が解消するか確認。
   解消すれば root cause は (A)/(B) のデプロイ/キャッシュ層で確定。
3. **CF キャッシュパージ**（解消しない場合）: Pages のキャッシュ無効化を実施。
4. 上記で解消した場合でも、再発防止として **CI に site ビルド検証 workflow を追加**することを別タスクとして提案
   （現状 `.github/workflows/` に site ビルドの CI ゲートが無く、壊れたデプロイを事前に検知できない）。

**方針B: ソース堅牢化を先行（500 の直接原因でない可能性が高いが、保険）**
- getting-started 固有の構文を疑い続けるより、まず CF 側を確認すべき（ローカル & Docker で再現しないため）。
- ただし副次的改善として: sync-wiki.mjs の prebuild が CF で部分失敗しても全体ビルドを止めるよう
  `astro build` 前に生成ファイル数の検証（期待33ページ）を追加する案。これは方針Aの再発防止策と重複。

**方針C: ホスティング非依存の検証用 CI 追加のみ（最小）**
- `.github/workflows/` に `site/` の `npm ci && npm run build` を回す PR ゲートを追加し、
  以後のデプロイ前にビルド健全性を機械検証する。500 の即時解消にはならないが再発防止になる。

### 5.4 architect / developer への引き継ぎ案（承認後に確定）

- **architect**: SPEC/ARCHITECTURE 不在のため設計ドキュメント更新は N/A。
  方針Aの「再発防止 CI」を採用する場合のみ、`.github/workflows/site-build.yml` の最小設計を委譲。
- **developer（または直接ユーザー作業）**:
  - CF Pages ダッシュボード確認（ユーザー依頼必須・権限外）
  - 再デプロイ/キャッシュパージのトリガ
  - 採用時: site ビルド検証 workflow の実装 + sync-wiki 生成ファイル数検証

### 5.5 ユーザーへの確認依頼事項（root cause 確定に必須）

analyst-core/architect は Cloudflare Pages ダッシュボードおよび本番 HTTP ヘッダに直接アクセスできないため、
以下はユーザー側での確認が必要:

1. CF Pages 最新デプロイの **ビルドログ全文**（prebuild と astro build の成否、生成ページ数）
2. **デプロイ済み commit SHA**（現 main HEAD と一致するか）
3. 本番 `/en/getting-started/` の **レスポンスヘッダ**（`cf-cache-status`, `server`, エラーボディの文言）
   — 500 が CF のキャッシュ由来か、ビルド成果物欠落由来かを切り分けるため
4. （任意）再デプロイ実施後に 500 が解消するか
