...
> Updated: 2026-05-31（analyst-core 深掘り分析・方針は承認待ちの「案」）

## §5 深掘り分析（analyst-core）

> 本節は site/ 配下の実コードを調査した結果。方針は承認ゲート前のため「案」として記載する。
...
- **セキュリティ**: `target="_blank"` には reverse tabnabbing 対策として
  ...
  を明示指定する。
- **a11y / UX（要承認確認）**: 新しいタブで開く旨を視覚的に示すか（外部リンク
  アイコン `↗` の付与 = `content` オプション）は任意。今回は対象リンクが 4 件と
  少なく、また既存 wiki のデザインに視覚ノイズを増やさない観点から
  **初期実装ではアイコン非付与（target/rel のみ）を案とする**。必要なら
  follow-up で `content` オプション追加。
...
2. `site/astro.config.mjs` で `rehype-external-links` を import し、
   ...
   ```
3. 外部リンクアイコン（`content`）は初期実装では付与しない（5.4）。
...
## §8 architect への引き継ぎ（案）

> 承認後に確定。本プロジェクトには ARCHITECTURE.md が存在せず、変更は
> `site/astro.config.mjs` の rehype プラグイン 1 行追加 + 依存 1 件追加に閉じる
> 小規模対応。architect が扱う設計ドキュメント更新は無い。

- 実装タスク（architect → developer 想定）:
  1. `cd site && npm install rehype-external-links@^3.0.0`（依存追加）
  2. `astro.config.mjs` の `markdown` に `rehypePlugins` を追加（§6.2）
  3. `npm run build` でビルド成功を確認、`site/dist/` の wiki ページ HTML で
     外部 GitHub リンクに `target="_blank" rel="noopener noreferrer"` が
     付与され、内部相対リンクには付与されないことを検証
